...
> [!NOTE]
> 계정 시스템이 암호화 프로토콜을 사용한다면, 상황에 맞는 추가 작업이 필요할 수 있습니다. 대표적으로 사설 인증서를 사용한다면 인증서 등록을 위한 작업이 필요합니다. 해당 과정은 에티버스와 함께 진행하여야 합니다.
...
연동하는 시스템간 정상적인 필드 매핑이 이루어지지 않는다면, 정상적인 SSO 인증과정을 수행 할 수 없습니다.
...
> [!NOTE]
> 오브리움에서 계정시스템으로 연동한 클라우드 사용자 계정으로 수행

<p align="center"><img src="images/orb-fp-01.png" width="75%" /><br/>< 로그인 화면 ></p>

<p align="center"><img src="images/orb-fp-04.png" width="75%" /><br/>< 프로젝트 생성 ></p>

프로젝트 드롭박스를 눌러 하단의 `프로젝트 생성` 버튼을 확인합니다. 색상이 무채색이면 프로젝트 생성 권한이 없는 상태입니다. 색상인 나타난다면 정상적으로 프로젝트 생성부터 시작하여 클라우드 작업을 수행 할 수 있습니다. 만약 프로젝트 생성권한이 없다면 다음을 확인합니다.

- VCF5 환경: Aria Automation의 프라임 프로젝트 `project` 에 사용자가 `Member`로 등록되어 있는지 확인합니다.
- VCF9 환경: 준비중...

<p align="center"><img src="images/orb-fp-03.png" width="75%" /><br/>< 권한 갱신 요청 ></p>

만약 프로젝트 생성 권한을 새로 등록한 계정이라면, 환면에 `권한 갱신 요청` 알람이 발생하고 화면을 재시작 하여 권한을 반영합니다.

> [!NOTE]
> 오브리움의 모든 프로젝트 권한은 VMware 서비스의 권한 그룹과 동일하게 설정됩니다. 단 오브리움 관리자 페이지에서 등록한 클라우드 관리자 계정은 모든 프로젝트에 대한 접근 권한을 가집니다.
